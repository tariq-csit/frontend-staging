...
function Sidebar() {
  ...
  return (
    <div
      className={`flex w-[17.5rem] duration-75 ${
        collapsed && "w-[4.75rem]"
      } h-screen flex-col justify-between items-start shrink-0 rounded-component`}
    >
      <div className="flex flex-col justify-center gap-6 self-stretch ">
        <div className="flex p-component items-center gap-[0.75rem] self-stretch">
          <img
            className={`${collapsed && "scale-[3.0]"}`}
            src={collapsed ? smallLogo : largeLogo}
          />
          <div
            className={`border cursor-pointer border-secondary duration-200 bg-white flex p-1 items-center gap-[0.625rem] rounded-full absolute ${
              collapsed ? "left-16 rotate-180" : "left-[16.7rem]"
            }  top-7`}
        ...
        <div
          className={`'flex flex-col mx-auto items-start duration-100 gap-component ${
            collapsed ? "w-[2.75rem]" : "w-[14.625rem]"
          }`}
        >
